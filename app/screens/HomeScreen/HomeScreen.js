...
class HomeScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    AsyncStorage.getItem("bareer_key").then(res => {
      if (res) {
  ...
  async componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.fetchService.status_code === "5001") {
      ...
      this.setState({
        sellerData: nextProps.fetchService.response.items,
        loader: false
      });
  ...
  _renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        onPress={() =>
          this.props.navigation.navigate("ProuductDetails", {
            passdata: item,
            token: this.state.bareer_key
          })
        }
      >
        ...
        <View style={container}>
          {!this.state.isSearch ? (
            <View style={styles.searchView}>
              ...
              <TouchableOpacity
                onPress={() =>
                  this.props.action.searchSeller(
                    this.state.bareer_key,
...
const mapDispatchToProps = dispatch => {
  return {
    action: bindActionCreators(
      { sellerList, searchSeller, ClearAction },
      dispatch
    )
  };
};
