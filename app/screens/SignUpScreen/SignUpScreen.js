...
import { signUp, getSignUpOtp, getToken } from "../../config/fetchApi";
...
class SignUP extends React.Component {
  ...
  TokenApiResponse = response => {
    ...
    if (response != null) {
      ...
      if (response.data.status == 200) {
        this._hitGetSignUpOtp(response.data.response.token);
      } else {
        setTimeout(() => {
  ...
  _hitGetSignUpOtp = async token => {
    ...
    await getSignUpOtp(
      ...
      this.state.password,
      token,
      response => this.getSignUpOtpResponse(response)
    );
  };

  getSignUpOtpResponse = response => {
    ...
    if (response != null) {
      ...
      if (response.data.status == 200) {
        this.props.navigation.navigate("VerficationScreen", {
