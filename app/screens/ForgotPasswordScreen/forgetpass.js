...
import { login, getToken, getForgetPassOtp } from "../../config/fetchApi";
...
class ForgetPass extends Component {
  ...
  TokenApiResponse = response => {
    ...
    if (response != null) {
      ...
      if (response.data.status == 200) {
        this._hitGetForgotPassOtp(response.data.response.token);
  ...
  _hitGetForgotPassOtp = async token => {
    debugger;
    await getForgetPassOtp(this.state.phoneNumber, token, response =>
      this.getForgotPassOtpResponse(response, token)
    );
  };
  ...
  getForgotPassOtpResponse = (response, token) => {
    ...
    if (response != null) {
      ...
      if (response.data.status == 200) {
        ...
        this.props.navigation.navigate("ForgotPasswordVerficationScreen", {
